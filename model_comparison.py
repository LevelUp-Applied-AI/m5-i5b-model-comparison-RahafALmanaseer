...
def plot_pr_curves_top3(models, X_test, y_test, output_path="results/pr_curves.png"):
    ...
    scores = {name: average_precision_score(y_test, md.predict_proba(X_test)[:, 1]) for name, md in models.items()}
...
def log_experiment(results_df, output_path="results/experiment_log.csv"):
    ...
    log_df = pd.DataFrame()
    log_df['model_name'] = results_df['model']
    log_df['accuracy'] = results_df['accuracy_mean']
    log_df['precision'] = results_df['precision_mean']
    log_df['recall'] = results_df['recall_mean']
    log_df['f1'] = results_df['f1_mean']
    log_df['pr_auc'] = results_df['pr_auc_mean']
